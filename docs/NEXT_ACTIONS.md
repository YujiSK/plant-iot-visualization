...
5. Slack AppのEvent subscriptionと`SLACK_OBSERVATION_CHANNEL_ID`を設定し、`#plant-observation`の実写真投稿から`care_logs`作成とSlack返信を確認する
6. 今後の給水・植え替え作業を`care_logs`に記録できる入力方法を追加する
7. BH1750を青色蓋のタッパー内部から透明な防水ケースへ移し、葉と同程度の高さ・空向きで設置する
8. BH1750移設後、同一時間帯・天候で移設前後の値を比較し、透明ケースの透過損失と補正係数を確認する
9. BH1750の5分値から日中積算luxを算出し、透明ケース移設後の傾向評価へ利用する
...
- Slack写真観察ログの次段階として、OpenAI Visionによる発芽、子葉、本葉、水位確認などのAI観察支援を追加する
